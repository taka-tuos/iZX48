...
{																		\
        (x) = io80_readm(NULL, (address) & 0xffff);						\
}
...
{																		\
		(x) = (int)io80_readm(NULL, (address) & 0xffff)						\
				| ((int)io80_readm(NULL, ((address) + 1) & 0xffff) << 8);	\
}
...
{																		\
		io80_writem(NULL, (address) & 0xffff, (x));\
}

#define Z80_WRITE_WORD(address, x)										\
{																		\
		io80_writem(NULL, (address) & 0xffff, (x) & 0xff);				\
		io80_writem(NULL, ((address) + 1) & 0xffff, (x) >> 8);				\
}
...
{																		\
		(x) = io80_readp(NULL, (port));							\
}

#define Z80_OUTPUT_BYTE(port, x)										\
{																		\
		io80_writep(NULL, (port), (x));							\
}
